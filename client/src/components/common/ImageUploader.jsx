  ...
  (
    ...
    return (
      <div className="w-full">
        <p className="font-medium text-sm text-gray-700 mb-3">{title}</p>
        ...
        {previewUrls.length === 0 && (
          <div
            className={`border-2 border-dashed rounded-xl flex flex-col items-center justify-center py-8 cursor-pointer transition-all duration-200 ${
              isDragging
                ? "border-orange-500 bg-orange-50 shadow-[0_0_0_4px_rgba(251,146,60,0.2)]"
          ...
          >
            ...
            <div className="flex flex-col items-center text-center select-none">
              <div className="flex items-center justify-center w-12 h-12 rounded-full bg-orange-100 mb-2">
        ...
        {previewUrls.length > 0 && (
          <div className="grid grid-cols-3 gap-3">
            {previewUrls.map((url, i) => (
              <div
                key={i}
                className="relative group overflow-hidden rounded-lg shadow-sm border bg-white"
              >
                <img
                  src={url}
                  alt={`preview-${i}`}
                ...
                <button
                  ...
                  className="absolute top-1 right-1 bg-white/90 rounded-full p-1 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                  title="Remove"
                >
            ...
            {previewUrls.length < maxFiles && (
              <>
                ...
                <button
                  type="button"
                  onClick={openFileDialog}
                  className="border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center hover:border-orange-400 hover:bg-orange-50 transition-colors"
